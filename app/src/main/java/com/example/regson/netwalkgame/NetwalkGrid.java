...
class NetwalkGrid {
  ...
  private final int mRows;
    ...
    mGridCopy = new int[columns * rows];
        int mServerPos = generate();

  }
    /**
     *scramble the grid all into different directions
     */
    private void scramble() {
        ...
        for (int row = 0; row < GameActivity.DEFAULT_ROWS; row++) {
            for (int col = 0; col < GameActivity.DEFAULT_COLUMNS; col++) {
                int rotateRand = random.nextInt(3);
                for (int rotateCounter = 0; rotateCounter < rotateRand; rotateCounter++) {
                    rotateRight(col, row);
                }
            }
        }
    ...
    /**
     *create copy of the grid to be used to compare if player's answer is same
     */
    private void createCopy(){
        System.arraycopy(mGrid, 0, mGridCopy, 0, mGrid.length);
  }
    /**
     *check if the game has been won, all the grid piece matches the original
     */
  private void checkWin() {
      boolean win=true;
      for (int pos=0; pos <mGrid.length; pos++) {
        if (mGridCopy[pos]!=mGrid[pos]){
          win=false;
        }
      }
      if (win){
  ...
  private int generate() {
    ...
    for (int i = mGrid.length - 1; i >= 0; --i) {
      switch (mGrid[i]) { // the server has an extra bit so will not match anywa
        case LEFT:
        case RIGHT:
        case UP:
        case DOWN:
  ...
  private boolean canExtendFrom(final int pos) {
  ...
  public boolean isNode(int x, int y) {
    ...
    public boolean isConnected(int x, int y){
        int gridpos = gridPos(x,y);
        return (mGrid[gridpos]& CONNECTED) !=0;
    }
